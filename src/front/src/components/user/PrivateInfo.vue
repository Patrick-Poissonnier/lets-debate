...
//      console.log('saveText : ' +this.$refs.presentation.editor.isDirty())
      ...
      axios.post( '/user/update/', {presentation: this.userInfo.presentation} )
      .then( response => {
    ...
    refreshCanvas() {
      ...
      this.ctx.translate( (128 -this.img.width*scale)/2, (128- this.img.height*scale)/2)
    ...
    saveAvatar() {
      ...
      this.$refs.preview.toBlob( blob => {
        ...
        axios.post('/user/newAvatar', formData, config)
        .then( response => {
          if( typeof response.data === "object") {
            this.changeAvatar= false
            this.$emit('updateUser',  response.data)
