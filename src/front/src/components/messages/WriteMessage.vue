<template>
  <b-modal id="WriteMessage" size="xl" @change="update" title="Votre réponse">
    <form>
      <div
        v-if="isAdmin"
        style="display: grid; grid-template-columns: repeat(3, 4r)"
      >
        <div style="grid-column: 1">
          ...
          >
            ...
            <b-popover target="help1" triggers="hover" placement="top">
              ...
              Désactive tous les controles d'un message normal, seul le champ
              texte est affiché, à vous de créer les liens pour naviguer dans le
              site.
            </b-popover>
          </b-form-checkbox>
        </div>
        <div style="grid-column: 2">report</div>
        <div style="grid-column: 3">report - abusivReport</div>
        <p />
      </div>
      <div v-else>
        <textarea
      ...
      />
      <TinyEditor
        :hidden="!isAdmin || !isType(newType, 'resume')"
        v-model="newText.resume"
    ...
    <template #modal-footer>
      ...
      <b-button variant="alert" v-on:click="quite()"> quitter </b-button>
    </template>
  </b-modal>
</template>
...
export default {
  ...
  methods: {
    ...
    async register() {
      ...
      const newMessage = this.isAdmin
        ? this.getAdminMessage()
        : this.getNotAdminMessage();
      if (Object.keys(newMessage).length) {
        if (this.message) {
          this.result = await libMessage.updateMessage(
            this.message,
            newMessage
          );
        } else {
          newMessage.parentId = this.result = await libMessage.createMessage(
            this.parentId,
            newMessage
          );
        }
