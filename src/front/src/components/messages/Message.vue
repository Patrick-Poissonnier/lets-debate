...
<div v-if="message" class="gridMessage">
  <div :class="responseType" :style="messageStyle"
    :key="message.id">
    <div class="gridEntete">
      <div class="avatar">
        <AuthorAvatar :pseudo="message.authorPseudo" 
          :interest="true" 
      ...
      <div class="title">
        <div v-if="isChild" class="titleText">
          <a :href="uri('Debate', {id : message.id})" 
            @click.prevent="goChild">
            <span v-html="message.text.title"/>
          </a>
        </div>
        <div v-else class="titleText" v-html="message.text.title"/>
        <div v-if="message.text.adminText" class="adminText" v-html="message.text.adminText"> 
        </div>
        <div v-if="message.text.resume" class="textResume" v-html="message.text.resume"> 
        </div>
      </div>
      <div class="right">
        <div class="rightButton">
          ...
          >Admin</b-button>
          <b-button v-if="showText" style="text-align: 'right'"
        ...
        <div v-if="isType(message.type, 'report')">
          <b-img  
            ...
            src="/logo/signe-attention.png" width="32" 
          />
          <b-popover :target="message.id+'popover-report'" triggers="hover" placement="bottom">
    ...
    <div v-if="showText" v-html="message.text.text">
    </div>
      
    <div v-if="!isChild" class="bottom">
      <span>
        {{message.nbResponse}} réponses.  triés par :
      </span>
      <b-dropdown :text="tabSort[sortedBy]" variant="outline-primary" size='sm'>
        <b-dropdown-item-button  @click="sortBy(1)">{{tabSort[1]}}</b-dropdown-item-button>
        <b-dropdown-item-button  @click="sortBy(2)">{{tabSort[2]}}</b-dropdown-item-button>
        <b-dropdown-item-button  @click="sortBy(3)">{{tabSort[3]}}</b-dropdown-item-button>
      </b-dropdown>&nbsp; 
      <b-button v-if="isType(message.type, 'evaluate') || isType(message.type, 'report')" id="evaluate-btn"
        variant="outline-primary" size='sm'
        @click="connected && chowModalEvaluate( {message, callBack: cbNewVote})">
        Evaluer
      </b-button>&nbsp;
      <b-popover v-if="isType(message.type, 'evaluate') || isType(message.type, 'report')&&!connected" 
        target="evaluate-btn" placement="top" triggers="hover">
          Vous devez vous connecter d'abord
      </b-popover> 
      <b-button v-if="replyAllowed" id="reply-btn"
        variant="outline-primary" size='sm'
        @click="showWriteMessage()">
        répondre
      </b-button>&nbsp;
      <b-popover v-if="replyAllowed&&!connected" 
        target="reply-btn" placement="top" triggers="hover">
          Vous devez vous connecter d'abord
      </b-popover> 
      <b-popover v-if="replyAllowed&&connected&&!message.myVote" 
        target="reply-btn" placement="top" triggers="hover">>
          Vous devez évaluer ce message<br/>avant d'y répondre
      </b-popover> 
      <b-button v-if="message.parentId"
        variant="outline-primary" size='sm'
        @click="goParent">
        retour
      </b-button>&nbsp;
    ...
    <div v-else class="bottom">
      <span>
        {{message.nbResponse}} réponses.
      </span>
      <b-button
        variant="outline-primary" @click="goChild" size='sm'>
        Entrer
      </b-button>
    </div>
  </div>
  ...
  computed: {
    ...
    messageStyle() {
      return {
        ...
        borderColor: this.bgColor( this.message.myVote && this.message.myVote.vote).color,
    ...
    responseType() {
      ...
      if( this.isChild ) {
        switch (this.message.responseType) {
          case 3 : result = {gridDisagree: true}
            break
          case 4 :result = {gridAgree: true}
          break
          default: result = {gridNone: true}
        }
      } else {
